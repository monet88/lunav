...
function hasExpectedRedirectType(
  ...
  // Confirmation emails use redirectType null (not a non-null type string).
...
/**
 * Completes a PKCE auth callback for the expected flow only.
 * Never logs the request URL, code, session, or provider error details.
 */
